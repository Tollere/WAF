...
var path = require('path'), fs = require('fs'), url = require('url'), mime = require('mime'), showDir = require('./ecstatic/showdir'), version = JSON.parse(fs.readFileSync(__dirname +
                                                                                                                                                                            '/../package.json').toString()).version, status = require('./ecstatic/status-handlers'), etag = require('./ecstatic/etag'), optsParser = require('./ecstatic/opts');
...
var ecstatic = module.exports = function (dir, options){
  ...
  var root = path.join(path.resolve(dir),
                       '/'), opts = optsParser(options), cache = opts.cache, autoIndex = opts.autoIndex, baseDir = opts.baseDir, defaultExt = opts.defaultExt, handleError = opts.handleError;
  ...
  return function middleware(req, res, next){
    ...
    var file = path.normalize(path.join(root, path.relative(path.join('/', baseDir), pathname))), gzipped = file +
                                                                                                            '.gz';
...
if (!module.parent){
  var http = require('http'), opts = require('optimist').argv, port = opts.port || opts.p || 8000, dir = opts.root ||
                                                                                                         opts._[0] ||
                                                                                                         process.cwd();
