...
var assert = require('assert'), bodyParser = require('connect').bodyParser, request = require('request'), vows = require('vows'), union = require('../');
...
vows.describe('union/body-parser').addBatch({
                                              "When using union with connect bodyParser()" : {
                                                ...
                                                "a request to /" : {
                                                  ...
                                                  "should respond with a body-decoded object" : function (err, res,
                                                                                                          body){
                                                    assert.isNull(err);
                                                    assert.equal(res.statusCode, 200);
                                                    assert.deepEqual(JSON.parse(body), { a : 'foo', b : 'bar' });
                                                  }
                                                }
                                              }
                                            }).export(module);
