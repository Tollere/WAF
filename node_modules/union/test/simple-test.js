...
var assert = require('assert'), fs = require('fs'), path = require('path'), spawn = require('child_process').spawn, request = require('request'), vows = require('vows'), macros = require('./helpers/macros');

var examplesDir = path.join(__dirname, '..', 'examples', 'simple'), simpleScript = path.join(examplesDir,
                                                                                             'simple.js'), pkg = JSON.parse(fs.readFileSync(path.join(__dirname,
                                                                                                                                                      '..',
                                                                                                                                                      'package.json'),
                                                                                                                                            'utf8')), fooURI = 'http://localhost:9090/foo', server;
                                         ...
                                         "When using union" : {
                                           "a simple http server" : {
                                             ...
                                             "a GET request to `/foo`"             : {
                                               ...
                                               "it should respond with `hello world`"                     : function (err,
                                                                                                                      res,
                                                                                                                      body){
                                                 macros.assertValidResponse(err, res);
                                                 assert.equal(body, 'hello world\n');
                                               },
                                               "it should respond with 'x-powered-by': 'union <version>'" : function (err,
                                                                                                                      res,
                                                                                                                      body){
                                                 assert.isNull(err);
                                             ...
                                             "a POST request to `/foo`"            : {
                                               ...
                                               "it should respond with `wrote to a stream!`" : function (err, res,
                                                                                                         body){
                                                 macros.assertValidResponse(err, res);
                                             ...
                                             "a GET request to `/redirect`"        : {
                                               ...
                                               "it should redirect to `http://www.google.com`" : function (err, res,
                                                                                                           body){
