...
Request.prototype.init = function (options){
    ...
    if (http.globalAgent && self.uri.protocol === "https:"){
      ...
      var tunnelOptions = { proxy : { host : self.proxy.hostname, port : +self.proxy.port, proxyAuth : self.proxy.auth, headers : { Host : self.uri.hostname +
                                                                                                                                           ...
                                                                                                                                           (
                                                                                                                                             ...
                                                                                                                                             'https:' ?
                                                                                                                                             443 :
                                                                                                                                             80) }}, rejectUnauthorized : self.rejectUnauthorized, ca : this.ca }
  ...
  if (options.auth){
    self.auth((options.auth.user === "") ? options.auth.user : (options.auth.user || options.auth.username ),
              options.auth.pass || options.auth.password, options.auth.sendImmediately)
  }
...
Request.prototype.aws = function (opts, now){
  ...
  var auth = { key : opts.key, secret : opts.secret, verb : this.method.toUpperCase(), date : date, contentType : getHeader('content-type',
                                                                                                                            this.headers) ||
                                                                                                                  '', md5 : getHeader('content-md5',
                                                                                                                                      this.headers) ||
...
request.head = function (uri, options, callback){
  ...
  if (params.options.body || params.options.requestBodyStream ||
      (params.options.json && typeof params.options.json !== 'boolean') || params.options.multipart){
